...
Odległość Euklidesowa okreslona jest wzorem:
\begin{equation}
    d(x,y)=\sqrt{(x_{1}-y_{1})^{2}-(x_{n}-y_{n})^{2}}
\end{equation}
gdzie $d$ - miara odległości, $x$, $y$ - wartość artykułów

\subsubsection{Metryka Czybyszewa}
Odległość Czybyszewa określa się wzorem:
\begin{equation}
...
Odlagłość ta wyraża się wzorem:
\begin{equation}
    d(x,y)=\sum |x_{i}-y_{i}|
\end{equation}
gdzie $d$ - miara odległości, $x$, $y$ - wektory wartości artykułów
...
Odlagłość ta polega na sumie różnicy elementów.\\
Wyrażone wzorem:
\begin{equation}
    d(x,y)=\sum (x_{i}-y_{i})
\end{equation}

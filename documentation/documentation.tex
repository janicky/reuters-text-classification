...
Cecha to liczba całkowita przyjmująca wartości z przedziału $<0; n>$ gdzie $n$ liczba słów w tekście.
\begin{equation}
    F=S_{1} + ... + S_{n}
\end{equation}
gdzie:\\
...
Cecha to liczba przyjmująca wartości z zakresu $<0; 1>$.
    \begin{equation}
      F=\frac{S}{L}
    \end{equation}
gdzie:\\
...
Najczęściej występujące słowo kluczowe wyrażone jako tekst odpowiadający najwyększej liczbie wystąpień danego
...
na podstawie następujących algorytmów:
%\begin{itemize}
%    \item podane przez użytkownika;
%    \item wczytane w pliku;
%    \item wygenerowane na podstawie zbioru uczączego;
%\end{itemize}
%Generowanie słów kluczowych odbywa się za pomocą poniżej zaprezentowanych metod.
